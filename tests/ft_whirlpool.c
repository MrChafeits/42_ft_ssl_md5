...
/*
** TODO: whrl_init, whrl_update, whrl_final
*/

#define WHRL_CNT (256 / 8)
#define WHRL_BBLOCK (512)
#define ROUNDS 10
#define ROL(I,N) ((I) << (N) ^ (I) >> (64 - N))
#define N 1
#define LL(c0,c1,c2,c3,c4,c5,c6,c7) c0,c1,c2,c3,c4,c5,c6,c7
#define C0(K,i) (Cx.q[K.c[(i) * 8 + 0]])
#define C1(K,i) ROL(Cx.q[K.c[(i) * 8 + 1]], 8)
#define C2(K,i) ROL(Cx.q[K.c[(i) * 8 + 2]], 16)
#define C3(K,i) ROL(Cx.q[K.c[(i) * 8 + 3]], 24)
#define C4(K,i) ROL(Cx.q[K.c[(i) * 8 + 4]], 32)
#define C5(K,i) ROL(Cx.q[K.c[(i) * 8 + 5]], 40)
#define C6(K,i) ROL(Cx.q[K.c[(i) * 8 + 6]], 48)
#define C7(K,i) ROL(Cx.q[K.c[(i) * 8 + 7]], 56)

typedef union u_bep	t_bep;
union u_bep
{
	t_u8	c[(256 * N + ROUNDS) * sizeof(t_u64)];
	t_u64	q[(256 * N + ROUNDS)];
};
static const t_bep Cx = {
	{
...
/*
** TODO: getting these to be norme compliant will be annoying
**  add link to norme pdf or something so those unacquainted with its...
**  unique design, may learn of Gaetan Juvin's challenging coding restrictions
*/

void	ft_whrl_block(t_whrl_ctx *ctx, const void *inp, size_t n)
{
	int			r;
	const t_u8	*p = inp;
	union {
...
void	ft_whrl_bitupdate(t_whrl_ctx *c, const void *_inp, size_t bits)
{
	size_t		n;
	t_u32		bitoff;
	t_u32		bitrem;
	...
	bitoff = c->bitoff;
	bitrem = bitoff % 8;
	inpgap = (8 - (t_u32)bits % 8) & 7;
	c->bitlen[0] += bits;
	if (c->bitlen[0] < bits)
	{
		n = 1;
		while (c->bitlen[n] == 0 && ++n < (WHRL_CNT / sizeof(size_t)))
			c->bitlen[n]++;
	}
	while (bits)
	{
		t_u32 byteoff = bitoff / 8;
		t_u8 b;
...
{
	static size_t	chunk = ((size_t)1) << (sizeof(size_t) * 8 - 4);
	const t_u8		*inp = _inp;

	while (bytes >= chunk)
	{
		ft_whrl_bitupdate(c, inp, chunk * 8);
		bytes -= chunk;
		inp += chunk;
	}
	if (bytes)
		ft_whrl_bitupdate(c, inp, bytes * 8);
}
...
void	ft_whrl_final(t_whrl_ctx *c, t_u8 *md)
{
	...
	if (bitoff)
		...
		c->data[byteoff] = 0x80;
	byteoff++;
	if (byteoff > (WHRL_BBLOCK / 8 - WHRL_CNT))
	{
		if (byteoff < WHRL_BBLOCK / 8)
			ft_memset(&c->data[byteoff], 0, WHRL_BBLOCK / 8 - byteoff);
		ft_whrl_block(c, c->data, 1);
		byteoff = 0;
	}
	if (byteoff < (WHRL_BBLOCK / 8 - WHRL_CNT))
		ft_memset(&c->data[byteoff], 0, (WHRL_BBLOCK / 8 - WHRL_CNT) - byteoff);
	p = &c->data[WHRL_BBLOCK / 8 - 1];
	i = -1;
	while (++i < WHRL_CNT / sizeof(size_t))
	{
		v = c->bitlen[i];
		j = -1;
		while (++j < sizeof(size_t) && (!(!(v >>= 8))))
			*p-- = (t_u8)(v & 0xff);
	}
	ft_whrl_block(c, c->data, 1);
	if (md)
